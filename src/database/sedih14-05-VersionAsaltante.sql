...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 15-05-2023 a las 08:57:05
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_inicioSemana` (IN `fechaConsulta` DATETIME, OUT `SALIDA` DATETIME)   BEGIN DECLARE SALIDA DATE;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_porTipoDeHabitacion` (IN `HotelID` INT)   BEGIN
SELECT
    idTipo,
    COUNT(*) as cantidad
FROM
...
INSERT INTO `registroocupacion` (`idHotel`, `fecha`, `nivelGeneral`, `tipoHabitacion`, `nivelHabitacion`, `categoria`, `costoPromedio`) VALUES
